...
def get_cmd():
    ...
    parser.add_argument("-m", "--model", default="DHBR", type=str, help="which model to use, options: DHBR")
...
def main():
    ...
    assert paras["model"] in ["DHBR"], "Pls select models from: DHBR"
    ...
    for lr, l2_reg, item_level_ratio, bundle_level_ratio, bundle_agg_ratio, embedding_size, num_layers, c_lambda, oh_c_lambda, c_temp, in \
            product(conf['lrs'], conf['l2_regs'], conf['item_level_ratios'], conf['bundle_level_ratios'], conf['bundle_agg_ratios'], conf["embedding_sizes"], conf["num_layerss"], conf["c_lambdas"], conf["oh_c_lambdas"], conf["c_temps"]):
        ...
        conf["oh_c_lambda"] = oh_c_lambda
        conf["c_temp"] = c_temp
        settings += [str(c_lambda), str(oh_c_lambda), str(c_temp)]
        ...
        # model
        if conf['model'] == 'DHBR':
            model = DHBR(conf, dataset.graphs).to(device)
        else:
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                if conf["aug_type"] == "ED" and (batch_anchor+1) % ed_interval_bs == 0: 
                    ED_drop = True
                uib_loss, c_loss, oh_c_loss = model(batch, ED_drop=ED_drop) 
                loss = uib_loss + conf["c_lambda"] * c_loss + conf["oh_c_lambda"] * oh_c_loss
                loss.backward()
                optimizer.step()

                loss_scalar = loss.detach()
                uib_loss_scalar = uib_loss.detach()
                c_loss_scalar = c_loss.detach()
                oh_c_loss_scalar = oh_c_loss.detach()
                run.add_scalar("loss_uib", uib_loss_scalar, batch_anchor)
                run.add_scalar("loss_c", c_loss_scalar, batch_anchor)
                run.add_scalar("loss_c_oh", oh_c_loss_scalar, batch_anchor)
                run.add_scalar("loss", loss_scalar, batch_anchor)

                pbar.set_description("epoch: %d, loss: %.4f, uib_loss: %.4f, c_loss: %.4f, oh_c_loss: %.4f" %(epoch, loss_scalar, uib_loss_scalar, c_loss_scalar, oh_c_loss_scalar))
...
def get_metrics(metrics, grd, pred, topks):
    ...
    for m, topk_res in tmp.items(): # m : "recall" / "ndcg" ; topk_res : {topk : res, }
        for topk, res in topk_res.items(): #res : [nomina, denorm]
